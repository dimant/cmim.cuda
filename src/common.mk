...
ifeq ("$(GCCVERSION)", "OK")
	...
	CC         := gcc -fPIC
	LINK       := g++ -fPIC
else ifeq ("$(GCCVERSION)","FIXED")
	NVCC       := $(CUDA_INSTALL_PATH)/bin/nvcc --compiler-bindir ./compiler-bindir/
	CXX        := g++-4.4 -fPIC
	CC         := gcc-4.4 -fPIC
	LINK       := g++-4.4 -fPIC
else
	NVCC       := $(warning Could not find GCC 4.4)$(shell exit 1)
	CXX        := $(warning Could not find GCC 4.4)$(shell exit 1)
	CC         := $(warning Could not find GCC 4.4)$(shell exit 1)
	LINK       := $(warning Could not find GCC 4.4)$(shell exit 1)
endif
...
ifeq ($(USECUDADYNLIB),1)
	...
	LIB += ${OPENGLLIB} $(PARAMGLLIB) $(RENDERCHECKGLLIB) ${LIB} -Bstatic
endif
...
ifneq ($(STATIC_LIB),)
	...
	TARGETDIR := $(LIBDIR)/$(BINSUBDIR)
	TARGET    := $(TARGETDIR)/$(EXECUTABLE)
	LINKLINE = $(LINK) -shared -Wl,-soname,$(TARGET) -o $(TARGET) $(OBJS) $(LIB)
else
